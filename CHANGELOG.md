...
### fixed
2026-07-18 : fixed | #dashboard | **CAUSA RAÍZ REAL** del bug de `payment_methods` (commit 4dab002d) — afecta a TODOS los widgets con `:filters="form"` (cash-flow, weekly-sales, sunat-status, low-stock, debtors, payment-methods, RowTop). El padre `index.vue` pasa `form` directo: `form.month_start` es Date object (`moment().toDate()`). Axios serializa Date como ISO 8601 (`2026-07-18T00:00:00.000Z`). Backend hace `Carbon::parse($month_start.'-01')` que es permisivo y devuelve día 18 (no inicio de mes) → "Sin cobros registrados" / "Sin ventas". El race-condition fix anterior (AbortController + fetchToken) defendía pero no atacaba la causa. **Fix correcto**: nuevo computed `formattedFilters` en `index.vue` que formatea fechas igual que `apiPayload()` (month_* → "YYYY-MM", date_* → "YYYY-MM-DD"). Los 7 widgets ahora reciben `:filters="formattedFilters"` con strings en el formato que espera el backend. **Empirical proof** (measure_pm_iso8601.php): `[A] month_start=2026-07-18T00:00:00.000Z → labels=0 total=0` vs `[B] month_start=2026-07 → labels=1 total=7583.30`. El AbortController+fetchToken del commit 4dab002d se mantiene como defensa en profundidad (sin daño, sin uso).
...
### changed
2026-07-17 : changed | #dashboard | Revert parcial del ajuste anterior (commit 6e9e030d) tras 2 nuevos problemas visuales reportados: (a) Stock por agotarse quedaba con margen arriba de 8px (`mt-2` del HTML) heredado de cuando vivía stacked bajo SUNAT — quitado del HTML, ya no aplica. (b) ¿Cómo me pagan? dejó de mostrar el donut cuando se aplicaron los cambios anteriores — el `align-self-start` que agregué defensivamente cambiaba el cross-axis del `d-flex flex-column`, reorganizaba el flex interno de `.pm-body` (chart + leyenda) y el apexchart no encontraba dimensiones estables. Restaurado `height: 100%` en `.pm-panel` y quitado `align-self-start` del wrapper en index.vue — el donut vuelve a renderizar. **Mantenido** el `.ls-list` max-height 100→350px del commit anterior (el usuario no reportó problema con eso, sí con el `mt-2`). Lección aprendida: cuando un widget pasa de stacked a standalone, hay que limpiar el wrapper class del HTML pero NO tocar el CSS scoped de tamaño a menos que se diagnostique la causa exacta.
2026-07-17 : changed | #dashboard | Ajuste de tamaño tras feedback visual del swap: (a) Stock por agotarse: `.ls-list` max-height 100→350px — la card en `col-xl-4` standalone era enorme verticalmente pero la lista estaba topada a 100px (solo cabían 4 items). Ahora caben ~15 items en pantalla sin scroll, alineado con el patrón de Top Clientes/Top Productos (425px). HENAVI pasa de "15 productos · ver 4" a "15 productos · ver todos". (b) ¿Cómo me pagan?: quitado `height: 100%` de `.pm-panel` — estiraba verticalmente dentro del `col-xl-4 d-flex flex-column` de fila 5, dejando mucho aire abajo del donut. Ahora usa altura natural. Defensivamente también se le pasa `align-self-start` desde el wrapper en index.vue. Sin cambios de backend ni de layout grid — solo CSS scoped de 2 widgets Vue.
2026-07-17 : changed | #dashboard | Swap visual: Stock por agotarse (LowStock) ahora vive en fila 4 col-xl-4 al lado de TopProductos y ¿Quién me debe? (era fila 5 stacked bajo SUNAT). ¿Cómo me pagan? (PaymentMethods) baja a fila 5 col-xl-4 stacked bajo SUNAT (era fila 4 col-xl-4 standalone). Razón: Stock por agotarse es señal operativa que pesa más que el desglose de métodos de cobro. PaymentMethods al bajar recibe clases `mt-2 mb-0` para mantener la separación con SUNAT (LowStock ya tenía esto en su CSS). Deudores pasa a `col-xl-12` cuando `dashboard_products=false` (ahora es el único widget de la fila 4). Sin cambios en backends ni en componentes Vue — solo template de index.vue.
