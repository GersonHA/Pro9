...
//        $all_documents = collect($all_documents)->sortBy('date_sort')->all();
    ...
    /*
     * Panel Auditoría de Caja — commit pro8 19e8fc86 (Mar 14 2026).
     * Migrado a Pro9 el 2026-07-21. Ver plan en
     * pro8/FacturoomBrain/CerebroFacturoom/Planes/modulos/caja/audit-panel/2026-07-21-plan-audit-panel.md
     */
    public function getBoxesForAudit()
    {
        // FIX OOM 128MB: cajas grandes con miles de cash_documents pueden reventar memoria
        @ini_set('memory_limit', '512M');
        @set_time_limit(0);
        try {
            // Traemos todas las cajas
            $cajas = \App\Models\Tenant\Cash::with('user')->orderBy('id', 'desc')->get();

            $formatted = $cajas->map(function($caja) {
                $estado = $caja->state ? '🟢 ABIERTA' : '🔴 CERRADA';

                // Validación para evitar que cajas antiguas sin fecha crasheen el sistema
                $fecha = 'Sin fecha';
                if ($caja->date_opening) {
                    $fecha = \Carbon\Carbon::parse($caja->date_opening)->format('d/m/Y');
                } elseif ($caja->created_at) {
                    $fecha = $caja->created_at->format('d/m/Y');
                }

                // Validación de usuario y número de referencia
                $usuario = $caja->user ? $caja->user->name : 'Administrador';
                $referencia = $caja->reference_number ? $caja->reference_number : $caja->id;
    ...
    public function getAuditTransactions($id)
    {
        // FIX OOM 128MB: HENAVI caja 5 = 6,533 cash_documents, eager loading de payments x doc
        // excede 128MB. 512M da holgura para cajas medianas; si >500k cash_documents
        // considerar paginación (no implementado — fuera de scope 19e8fc86).
        @ini_set('memory_limit', '512M');
        @set_time_limit(0);
        try {
            $cash = \App\Models\Tenant\Cash::with(['cash_documents.document.user', 'cash_documents.sale_note.user', 'cash_documents.document.payments.payment_method_type', 'cash_documents.sale_note.payments.payment_method_type'])->findOrFail($id);

            $transactions = [];

            foreach ($cash->cash_documents as $cash_document) {
                $doc = $cash_document->document ?? $cash_document->sale_note;
                if (!$doc) continue;

                $cliente = $doc->customer->name ?? 'Clientes - Varios';
                $nro_doc_cliente = $doc->customer->number ?? '99999999';
                $responsable = isset($doc->user->name) ? $doc->user->name : ($cash->user->name ?? 'Administrador');
                $documento = $doc->series . '-' . $doc->number;
                $fecha = $doc->date_of_issue->format('d/m/Y') . ' ' . $doc->time_of_issue;

                // 🧠 DETECCIÓN DE ESTADO: Verificamos si está formalmente anulado
                $es_anulado = in_array($doc->state_type_id, ['11', '13']);

                // Verificamos si el sistema ya generó pagos negativos por su cuenta
                $tiene_pagos_negativos = false;
                if (isset($doc->payments)) {
                    foreach ($doc->payments as $p) {
                        if ((float)$p->payment < 0) $tiene_pagos_negativos = true;
                    }
                }

                if (isset($doc->payments) && count($doc->payments) > 0) {
                    foreach ($doc->payments as $payment) {
                        $metodo = $payment->payment_method_type->description ?? 'Efectivo';
                        $monto = (float) $payment->payment;
                        $tipo = 'Venta';

                        // Si el pago ya es negativo, es un extorno natural (Ej: Notas de Venta)
                        if ($monto < 0) {
                            $tipo = 'Extorno / Anulación';
                        } elseif (isset($doc->document_type_id) && $doc->document_type_id === '07') {
                            $tipo = 'Nota de Crédito';
                            if ($monto > 0) $monto = -$monto;
                        }

                        // 1. Inyectamos la fila del Ingreso Original
                        $transactions[] = [
                            'id' => 'pay-' . $payment->id,
                            'datetime' => $fecha,
                            'type' => $tipo,
                            ...
                            'customer_number' => $nro_doc_cliente,
                            'method' => $metodo,
                            'amount' => $monto,
                        ];

                        // 2. MAGIA ANTI-EVASIÓN: Si está anulado, pero NO tiene pago negativo, lo creamos nosotros.
                        if ($es_anulado && !$tiene_pagos_negativos && $monto > 0 && $tipo !== 'Nota de Crédito') {
                            $transactions[] = [
                                'id' => 'pay-ext-' . $payment->id,
                                'datetime' => $fecha,
                                'type' => 'Extorno / Anulación',
                                'responsable' => $responsable,
                                'document' => $documento,
                                'customer_name' => $cliente,
                                'customer_number' => $nro_doc_cliente,
                                'method' => $metodo,
                                'amount' => -$monto, // FORZAMOS LA RESTA PARA CUADRAR CAJA
                            ];
                        }
                    }
                }

                // Detección de Notas de Crédito vinculadas
                if (method_exists($doc, 'getNotes')) {
                    $notes = $doc->getNotes();
                    if ($notes !== null) {
                        foreach($notes as $note) {
                            $doc_note = $note->getDocument();
                            if ($doc_note && in_array($doc_note->state_type_id, ['01', '03', '05', '07', '13'])) {
                                if ($doc_note->document_type_id === '07') {
                                    $monto_nota = (float) $doc_note->total;
                                    $metodo_nota = 'Efectivo';
                                    if (isset($doc->payments) && count($doc->payments) > 0) {
                                        $metodo_nota = $doc->payments->first()->payment_method_type->description ?? 'Efectivo';
                                    }

                                    $transactions[] = [
                                        'id' => 'nc-' . $doc_note->id,
                                        'datetime' => $doc_note->date_of_issue->format('d/m/Y') . ' ' . $doc_note->time_of_issue,
                                        'type' => 'Nota de Crédito',
                                        'responsable' => isset($doc_note->user->name) ? $doc_note->user->name : $responsable,
                                        'document' => $doc_note->series . '-' . $doc_note->number,
                                        'customer_name' => $cliente,
                                        'customer_number' => $nro_doc_cliente,
                                        'method' => $metodo_nota,
                                        'amount' => -$monto_nota,
